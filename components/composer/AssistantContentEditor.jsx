...
function Preview({ brandName, title, titlePosition, caption, hashtags, media }) { const positionClass = titlePosition === 'top' ? 'items-start' : titlePosition === 'center' ? 'items-center' : 'items-end'; return <aside className="lg:sticky lg:top-4"><p className="mb-2 text-xs font-bold uppercase tracking-wide text-muted">Prévia</p><div className="overflow-hidden rounded-2xl border border-line bg-surface"><div className="p-3 text-xs font-bold text-ink">{brandName}</div><div className={`relative flex aspect-square bg-surface-2 p-5 ${positionClass}`}>{media[0] && <img src={media[0]} alt="Prévia do conteúdo" className="absolute inset-0 h-full w-full object-cover" />}{title && <p className="relative max-w-full text-xl font-extrabold text-white drop-shadow-lg">{title}</p>}</div><p className="whitespace-pre-wrap p-3 text-xs text-ink"><b>{brandName}</b> {caption}<span className="text-accent"> {hashtags}</span></p></div></aside>; }
