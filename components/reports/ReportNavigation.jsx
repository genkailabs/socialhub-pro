  ...
  return (
    <nav aria-label="Canais do relatorio" className="flex flex-wrap gap-2">
      {channels.map(([channel, label]) => {
        ...
        return <Link key={channel} href={reportHref(channel)} className={`rounded-lg px-3.5 py-2 text-xs font-bold transition-colors ${active ? 'bg-accent text-black' : 'border border-line bg-surface text-muted hover:border-accent/60 hover:text-ink'}`}>{label}</Link>;
      })}
