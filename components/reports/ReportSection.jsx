  ...
  return (
    <section id={id} className={`scroll-mt-6 rounded-[22px] border bg-surface p-5 ${active ? 'border-accent ring-1 ring-accent/30' : 'border-line'}`}>
      <div className="flex items-start gap-3">
        ...
        {active && <span className="ml-auto rounded-full bg-accent/15 px-2.5 py-1 text-[10px] font-bold text-accent">Métrica do insight</span>}
      </div>
...
export function MissingMetric({ children = 'Ainda não há dados suficientes para esta métrica.' }) {
