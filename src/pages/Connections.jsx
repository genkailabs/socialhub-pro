...
import {
  ...
  MessageSquare, ShieldCheck, Sparkles, RefreshCw, X, Copy, Check,
  Cloud, Database, UserCog, Save, AlertTriangle, Unplug, Info
} from 'lucide-react';
...
export default function Connections({ setCurrentTab }) {
  ...
  useEffect(() => {
    ...
    if (status === 'success' && platform) {
      ...
      showBanner('error', `Erro na autorizacao: ${error}`, 8000);
      window.history.replaceState({}, document.title, window.location.pathname);
    }
  }, []);
          ...
          <div className="bg-[#111827] rounded-3xl max-w-lg w-full p-7 shadow-2xl border border-gray-800 text-white space-y-6 relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-5 bg-[#1F2937]/80 rounded-2xl border border-gray-700/80 space-y-4">
              ...
              {activeModalNet.id === 'instagram' && (
                <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl space-y-2.5">
                  <div className="flex items-center gap-2 text-sm font-bold text-amber-400">
                    ...
                    <span>Requisitos Obrigatorios da Meta (Instagram Business):</span>
                  </div>
                  <ul className="text-xs md:text-sm text-gray-200 space-y-1.5 list-disc list-inside">
                    ...
                    <li>Deve estar <strong>vinculado a uma Pagina do Facebook</strong> da qual voce seja Administrador.</li>
                  </ul>
              ...
              <div className="pt-2">
                ...
                <p className="text-xs text-gray-400 mb-4 leading-normal">
                  💡 Este nome e apenas para exibicao interna. Voce deve digitar o seu @ real.
                </p>
            ...
            <div className="p-5 bg-gradient-to-b from-[#0D1321] to-[#111827] rounded-2xl border border-emerald-500/20 space-y-4 relative overflow-hidden">
              ...
              <div className="flex items-center justify-between relative z-10">
                <span className="text-sm font-bold text-white flex items-center gap-2">
                  <UserCog className="w-4 h-4 text-emerald-400" /> 3. Conexao Manual (Preenchimento Direto)
                </span>
                <span className="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400 text-[10px] font-bold">Alternativo</span>
              </div>
              <p className="text-xs md:text-sm text-gray-300 leading-relaxed">
                Preencha os dados do seu perfil manualmente para registrar a conexao no painel.
              </p>

              <div className="space-y-3.5 relative z-10">
                <div>
                  <label className="block text-xs font-bold text-gray-300 uppercase tracking-wider mb-1.5">Nome de Usuario / @Handle *</label>
                  <input
                ...
                <div className="grid grid-cols-2 gap-3.5">
                  <div>
                    <label className="block text-xs font-bold text-gray-300 uppercase tracking-wider mb-1.5">Nome de Exibicao</label>
                    <input
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-300 uppercase tracking-wider mb-1.5">Bio / Descricao (opcional)</label>
                  <textarea
              ...
              <div className="p-3.5 bg-emerald-500/10 border border-emerald-500/30 rounded-xl flex items-start gap-2 relative z-10">
                ...
                <p className="text-xs text-emerald-300/90 leading-relaxed">
                  A conexao manual registra o perfil para gerenciamento no painel. Para analytics em tempo real, use a Opcao 1 (OAuth).
                </p>
              </div>
              ...
              >
                {isConnecting ? (<><RefreshCw className="w-4 h-4 animate-spin" /> Salvando...</>) : (<><Save className="w-4 h-4" /> Salvar Conexao Manual</>)}
              </button>
            </div>
          </div>
        </div>
                ...
                onClick={() => {
                  ...
                  if (activeBrand && !activeBrand.connectedChannels?.includes('whatsapp')) {
                    toggleChannelConnection(activeBrand.id, 'whatsapp', {
                      ...
                      expiresIn: 'Sessao Ativa (QR Code)', status: 'connected',
                      lastSynced: new Date().toLocaleString('pt-BR')
                    });
                  }
                  showBanner('success', `WhatsApp Business conectado a marca ${activeBrand?.name || 'ativa'}!`, 5000);
                }}
