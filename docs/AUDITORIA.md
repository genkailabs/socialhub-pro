...
| Hospedagem | Railway (`https://socialhub-mvp-production.up.railway.app`) |
...
2. `POLLINATIONS_SECRET_KEY` deve ser conferida no Railway e em `.env.local`; remover `GEMINI_API_KEY`/`TAVILY_API_KEY`/`DEAPI_API_KEY` se não forem usadas.
3. Redes sociais mockadas ainda a remover da UI.
4. Deploy do frontend está no Railway.
