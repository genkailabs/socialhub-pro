...
import { openrouterChat, DEFAULT_OPENROUTER_MODEL } from '@/lib/ai/openrouter';
import { estimateCostUsd } from '@/lib/ai/cost';
import { nextVersionNumber, canApprove, pickDnaColumns } from '@/lib/dna-versions';
import { nextMaxTokens } from '@/lib/ai/skills/run';
...
export async function analyzeBrandDNA({ brandId, brandName, wantIg, websiteUrl, pastedText, manual }) {
  ...
  try {
    ...
    // O DNA é a resposta mais longa que o produto pede: dna + report com
    // categorias, forças, fraquezas e oportunidades. Com o teto padrão de 1200
    // ele raspava o limite (a última geração bem-sucedida gastou 1033) e
    // qualquer marca com mais material voltava com o JSON cortado ao meio —
    // que é o "Resposta da IA não é JSON válido" visto na tela.
    //
    // Mesmo tratamento que lib/ai/skills/run.js já dá às outras skills: espaço
    // folgado e, se ainda assim cortar, uma segunda tentativa maior. Só o corte
    // justifica repetir; JSON inválido com resposta inteira é outro problema e
    // repetir só gastaria crédito.
    let maxTokens = 3000;
    let out = await openrouterChat({ system, user: userPrompt, jsonMode: true, maxTokens });
    if (out.finishReason === 'length') {
      maxTokens = nextMaxTokens(maxTokens);
      out = await openrouterChat({ system, user: userPrompt, jsonMode: true, maxTokens });
      if (out.finishReason === 'length') {
        throw new Error(`A IA não conseguiu fechar o Brand DNA dentro de ${maxTokens} tokens. Tente de novo com menos texto colado.`);
      }
    }
    ...
    await supabase.from('generation_jobs').insert({
      brand_id: brandId, user_id: user.id, kind: 'brand_dna', provider: 'openrouter', model: out.model,
    ...
    try {
      await supabase.from('generation_jobs').insert({
        brand_id: brandId, user_id: user.id, kind: 'brand_dna', provider: 'openrouter', model: DEFAULT_OPENROUTER_MODEL,
