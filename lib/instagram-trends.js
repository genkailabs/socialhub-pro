...
// Teto do material aceito pela rota do briefing (`app/api/carrossel/brief`).
// Material acima disso é recusado lá, e recusa depois de escolher a tendência
// pareceria defeito da escolha.
const MAX_SOURCE_MATERIAL = 6000;

/**
 * Tendência pesquisada → assunto e material de origem do carrossel.
 *
 * A análise de tendência é o tipo que exige fonte: escolher uma tendência tem
 * de carregar a evidência junto, senão o gerador recusa o roteiro logo adiante
 * e a pessoa não entende por quê. Só entram as fontes que a própria tendência
 * citou — anexar a pesquisa inteira convidaria o modelo a misturar assunto.
 */
export function tendenciaParaEntrada(trend, sources) {
  if (!trend || typeof trend !== 'object') return { topic: '', sourceMaterial: '', sources: [] };

  const citadas = Array.isArray(trend.sourceIds) ? trend.sourceIds : [];
  const usadas = (Array.isArray(sources) ? sources : []).filter((source) => citadas.includes(source.id));

  const partes = [
    trend.summary && `O que está acontecendo: ${text(trend.summary, 600)}`,
    trend.mechanic && `Mecânica: ${text(trend.mechanic, 800)}`,
    trend.howTo && `Como executar: ${text(trend.howTo, 900)}`,
    usadas.length && ['Fontes:', ...usadas.map((source) => (
      `- ${text(source.title, 200)}${source.publisher ? ` (${text(source.publisher, 80)})` : ''}${source.publishedAt ? ` — ${text(source.publishedAt, 40)}` : ''}: ${source.url}`
    ))].join('\n')
  ].filter(Boolean);

  return {
    topic: text(trend.carouselTheme || trend.title, 180),
