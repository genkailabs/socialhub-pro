...
// MVP V2 §1: o planejamento é uma JANELA MÓVEL de 7 dias que começa HOJE —
// nunca uma semana fechada que começa na próxima segunda. Se hoje é terça, a
// janela é terça→segunda.
//
// "Hoje" é o dia em São Paulo (UTC-3), não em UTC: é o fuso em que o produto
// pensa datas e horários (ver scheduleIsoFromPlanning). Sem esse ajuste, entre
// 21h e 0h de São Paulo o UTC já virou e a janela começaria no dia seguinte.
export function planningWindowStart(now = new Date()) {
  const sp = new Date(now.getTime() - 3 * 3600 * 1000);
  return iso(new Date(Date.UTC(sp.getUTCFullYear(), sp.getUTCMonth(), sp.getUTCDate())));
}

// As 7 datas da janela, em ordem. Base para "preencher só as datas vazias" (§4).
export function planningWindowDates(start = planningWindowStart()) {
  const base = new Date(`${start}T00:00:00.000Z`).getTime();
  return Array.from({ length: 7 }, (_, i) => iso(new Date(base + i * DIA_MS)));
}

// Datas da janela que ainda não têm item decidido nem ideia ocupando o dia.
// `occupied` são as datas já usadas; a IA só recebe o que sobrou (§4).
export function emptyWindowDates(start, occupied = []) {
  const taken = new Set((occupied || []).filter(Boolean).map((d) => String(d).slice(0, 10)));
  return planningWindowDates(start).filter((d) => !taken.has(d));
}
...
// Só um item efetivamente decidido ocupa vaga na semana. 'rejected' e 'idea'
// não contam — do contrário remover itens reduziria as vagas e a IA deixaria
// de sugerir substitutos (RF-01).
export const FILLED_ITEM_STATUSES = ['approved', 'in_production', 'ready'];

export function remainingPlanSlots(requestedPosts, items) {
  const filled = (items || []).filter((item) => FILLED_ITEM_STATUSES.includes(item?.status)).length;
  return Math.max(0, (requestedPosts || 0) - filled);
}
