...
export const STRUCTURES = [
  {
    ...
    category: 'visual',
    // `wide` (1.91:1) tem 566px de altura: só entram aqui as estruturas de foto
    // sangrada e a manchete. As demais empilham blocos que não cabem nesse quadro.
    shapes: ['square', 'portrait', 'story', 'wide'],
    slides: 1,
    density: 'airy',
  ...
  },
  {
    ...
    slots: [
      ...
      { component: 'fonte', x: 0, y: 0.84, w: 0.7, h: 0.04 },
  ...
  },
  {
    ...
    slots: [
      ...
      { component: 'subtitulo', x: 0, y: 0.7, w: 0.92, h: 0.09 },
      // Dado sem crédito não é dado, é palpite com número.
      { component: 'fonte', x: 0, y: 0.81, w: 0.7, h: 0.035 },
  ...
  // ajusta depois. Fingir detecção facial seria pior que assumir a regra.
  {
    ...
    slots: [
      ...
      { component: 'logo', x: 0, y: 0.93, w: 0.58, h: 0.05 },
      { component: 'fonte', x: 0.62, y: 0.93, w: 0.38, h: 0.05, overrides: { align: 'right' } }
    ]
  },
  {
    id: 'manchete-pessoa',
  ...
  },
  {
    ...
    slots: [
      ...
      { component: 'imagem-principal', x: 0, y: 0.13, w: 1, h: 0.34 },
      { component: 'titulo', x: 0, y: 0.5, w: 1, h: 0.22 },
      // Sem destaque a manchete saía plana — a palavra pintada é o que o olho
      // pousa antes de ler a frase inteira.
      { component: 'destaque-palavra', x: 0, y: 0.735, w: 0.5, h: 0.055, overrides: { align: 'left' } },
      { component: 'subtitulo', x: 0, y: 0.81, w: 0.92, h: 0.06 },
      { component: 'fonte', x: 0, y: 0.875, w: 0.7, h: 0.035 },
      { component: 'logo', x: 0, y: 0.93, w: 0.55, h: 0.05 },
      { component: 'data', x: 0.62, y: 0.93, w: 0.38, h: 0.05 }
    ]
  },
  {
    ...
    slots: [
      ...
      { component: 'fonte', x: 0, y: 0.86, w: 0.7, h: 0.035 },
      { component: 'logo', x: 0, y: 0.92, w: 0.6, h: 0.05 }
    ]
  },
  {
  ...
  // ------------------------------------------------------------------------
  // Lista visual e miolo de carrossel.
  //
  // O que faltava para um carrossel parecer uma peça editorial e não sete
  // repetições da mesma manchete: slides com FUNÇÃO diferente. Cada estrutura
  // daqui é um papel na sequência — explicar, listar, provar, concluir, chamar.
  //
  // Todas carregam a mesma moldura (`logo` embaixo à esquerda, `numero-slide`
  // à direita, nas mesmas coordenadas). O slot do número some sozinho num post
  // avulso, porque slot sem conteúdo não vira caixa fantasma — então a mesma
  // estrutura serve o feed e o miolo do carrossel sem uma cópia por caso.
  {
    id: 'lista-visual',
    label: 'Lista visual',
    description: 'Cada item no próprio cartão, com título dominante em cima.',
    category: 'educativo',
    shapes: ['square', 'portrait', 'story'],
    slides: 1,
    density: 'dense',
    contentTypes: ['educativo', 'servico', 'autoridade'],
    requires: { minBullets: 2 },
    recommendedFor: 'Lista que precisa ser lida de relance, item por item.',
    slots: [
      { component: 'selo-categoria', x: 0, y: 0, w: 0.42, h: 0.05 },
      { component: 'titulo', x: 0, y: 0.08, w: 1, h: 0.17 },
      // `panel` pinta o cartão; `onPanel` diz ao motor que a tinta agora é a de
      // cima do painel, e não a do fundo da peça.
      { component: 'lista', index: 0, x: 0, y: 0.29, w: 1, h: 0.145, panel: true, onPanel: true },
      { component: 'lista', index: 1, x: 0, y: 0.45, w: 1, h: 0.145, panel: true, onPanel: true },
      { component: 'lista', index: 2, x: 0, y: 0.61, w: 1, h: 0.145, panel: true, onPanel: true, optional: true },
      { component: 'lista', index: 3, x: 0, y: 0.77, w: 1, h: 0.145, panel: true, onPanel: true, optional: true },
      { component: 'logo', x: 0, y: 0.94, w: 0.6, h: 0.05 },
      { component: 'numero-slide', x: 0.7, y: 0.94, w: 0.3, h: 0.05 }
    ]
  },
  {
    id: 'lista-foto',
    label: 'Lista com foto',
    description: 'Foto no topo e os itens em cartões logo abaixo.',
    category: 'educativo',
    shapes: ['square', 'portrait', 'story'],
    slides: 1,
    density: 'dense',
    contentTypes: ['educativo', 'servico', 'noticia'],
    // O par com foto da `lista-visual`: mesma leitura, foto como abertura.
    requires: { image: true, minBullets: 2 },
    recommendedFor: 'Lista que abre com uma imagem de contexto.',
    slots: [
      { component: 'imagem-principal', bleed: true, x: 0, y: 0, w: 1, h: 0.36 },
      { component: 'titulo', x: 0, y: 0.41, w: 1, h: 0.14 },
      { component: 'lista', index: 0, x: 0, y: 0.58, w: 1, h: 0.14, panel: true, onPanel: true },
      { component: 'lista', index: 1, x: 0, y: 0.74, w: 1, h: 0.14, panel: true, onPanel: true },
      { component: 'logo', x: 0, y: 0.93, w: 0.6, h: 0.05 },
      { component: 'numero-slide', x: 0.7, y: 0.93, w: 0.3, h: 0.05 }
    ]
  },
  {
    id: 'slide-explicacao',
    label: 'Slide de explicação',
    description: 'Título curto, palavra em destaque e o texto que explica.',
    category: 'carrossel',
    shapes: ['square', 'portrait', 'story'],
    slides: 1,
    density: 'normal',
    contentTypes: ['educativo', 'autoridade', 'noticia'],
    requires: { subtext: true },
    recommendedFor: 'O slide de contexto, logo depois da capa.',
    slots: [
      { component: 'selo-categoria', x: 0, y: 0.02, w: 0.42, h: 0.05 },
      { component: 'titulo', x: 0, y: 0.12, w: 1, h: 0.2 },
      { component: 'destaque-palavra', x: 0, y: 0.35, w: 0.58, h: 0.07, overrides: { align: 'left' } },
      { component: 'subtitulo', x: 0, y: 0.47, w: 0.96, h: 0.34 },
      { component: 'logo', x: 0, y: 0.93, w: 0.6, h: 0.05 },
      { component: 'numero-slide', x: 0.7, y: 0.93, w: 0.3, h: 0.05 }
    ]
  },
  {
    id: 'slide-prova',
    label: 'Slide de prova',
    description: 'Print ou foto ocupando o miolo, com a leitura embaixo.',
    category: 'carrossel',
    shapes: ['square', 'portrait', 'story'],
    slides: 1,
    density: 'normal',
    contentTypes: ['noticia', 'dado', 'autoridade', 'educativo'],
    // A prova é a imagem: sem ela este slide não é uma versão pior de si mesmo,
    // simplesmente não existe.
    requires: { image: true, subtext: true },
    recommendedFor: 'Mostrar o print, o gráfico ou a foto que sustenta o argumento.',
    slots: [
      { component: 'selo-categoria', x: 0, y: 0.02, w: 0.42, h: 0.05 },
      { component: 'titulo', x: 0, y: 0.1, w: 1, h: 0.14 },
      { component: 'imagem-principal', x: 0, y: 0.27, w: 1, h: 0.45 },
      { component: 'subtitulo', x: 0, y: 0.75, w: 0.96, h: 0.13 },
      { component: 'logo', x: 0, y: 0.93, w: 0.6, h: 0.05 },
      { component: 'numero-slide', x: 0.7, y: 0.93, w: 0.3, h: 0.05 }
    ]
  },
  {
    id: 'slide-conclusao',
    label: 'Slide de conclusão',
    description: 'Painel de marca com a frase que fica e o destaque isolado.',
    category: 'carrossel',
    shapes: ['square', 'portrait', 'story'],
    slides: 1,
    density: 'airy',
    contentTypes: ['educativo', 'autoridade', 'inspiracao'],
    requires: {},
    // É o fecho: o título parte da escala de capa para pesar mais que o miolo.
    cover: true,
    recommendedFor: 'Fechar a sequência com a ideia que a pessoa leva.',
    slots: [
      { component: 'painel', bleed: true, x: 0, y: 0, w: 1, h: 1 },
      { component: 'selo-categoria', x: 0, y: 0.03, w: 0.44, h: 0.055 },
      { component: 'titulo', x: 0, y: 0.22, w: 1, h: 0.34 },
      { component: 'destaque-palavra', x: 0, y: 0.6, w: 0.6, h: 0.08, overrides: { align: 'left' } },
      { component: 'subtitulo', x: 0, y: 0.73, w: 0.94, h: 0.13 },
      { component: 'logo', x: 0, y: 0.93, w: 0.6, h: 0.05 },
      { component: 'numero-slide', x: 0.7, y: 0.93, w: 0.3, h: 0.05 }
    ]
  },
  {
    id: 'slide-cta',
    label: 'Slide de chamada',
    description: 'A ação esperada no centro, com a assinatura da marca.',
    category: 'carrossel',
    shapes: ['square', 'portrait', 'story'],
    slides: 1,
    density: 'airy',
    contentTypes: ['servico', 'promocao', 'engajamento', 'educativo'],
    requires: { fields: ['cta'] },
    cover: true,
    recommendedFor: 'O último slide: o que a pessoa faz agora.',
    slots: [
      { component: 'selo-categoria', x: 0, y: 0.03, w: 0.44, h: 0.055 },
      { component: 'titulo', x: 0, y: 0.24, w: 1, h: 0.26 },
      { component: 'subtitulo', x: 0, y: 0.55, w: 0.9, h: 0.12 },
      { component: 'cta', x: 0, y: 0.73, w: 0.72, h: 0.09 },
      { component: 'logo', x: 0, y: 0.93, w: 0.6, h: 0.05 },
      { component: 'numero-slide', x: 0.7, y: 0.93, w: 0.3, h: 0.05 }
    ]
  }
...
for (const structure of STRUCTURES) {
  const uses = { image: false, bullets: false, subtext: false, cta: false, source: false };
...
/**
 * Formato da peça, em quatro classes — e não em duas.
 *
 * `height > width ? 'story' : 'square'` colocava o 4:5 (1080×1350) e o 3:4
 * (1080×1440) na mesma caixa do Story 9:16 (1080×1920). São peças diferentes:
 * o Story tem a interface do Instagram por cima e quase o dobro de altura útil,
 * o 4:5 é o retrato do feed e é o formato recomendado hoje. Tratá-los igual
 * inflava margem, escala e posicionamento no formato que mais importa.
 *
 * Os cortes:
 * - até 1.05 de altura/largura → `square` (1:1);
 * - até 1.55                   → `portrait` (4:5 = 1.25 e 3:4 = 1.33);
 * - acima disso                → `story` (9:16 = 1.78);
 * - largura 1.2× a altura      → `wide` (1.91:1 = 0.52).
 */
export function shapeOf({ width = 1080, height = 1080 } = {}) {
  if (!width || !height) return 'square';
  if (width > height * 1.2) return 'wide';
  const aspect = height / width;
  if (aspect <= 1.05) return 'square';
  if (aspect <= 1.55) return 'portrait';
  return 'story';
}

/** Peça alta o bastante para a interface do app cobrir as bordas. */
export function isTallShape(shape) {
  return shape === 'story';
}
...
export function contentUsage(structure, content = {}) {
  ...
  // Crédito informado e estrutura que o mostra: notícia com fonte visível ganha
  // da mesma notícia sem fonte, que é o desempate certo para o tipo.
  if (String(content.source || '').trim()) points += uses.source ? 1 : 0;
