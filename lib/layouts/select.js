...
function scoreStructure(structure, { content, contentType, objective, format }) {
  let score = contentUsage(structure, content) * 3;

  if (structure.contentTypes?.includes(contentType)) score += 6;
  // "objetivo" e "nicho" chegam como texto livre; a categoria da estrutura é o
  // que aproxima os dois sem inventar uma taxonomia paralela.
  if (objective && deaccent(objective).includes(deaccent(structure.category))) score += 3;
...
export function selectStructure({
  content = {}, contentType = 'autoridade', objective = '', format = 'post',
  ...
  const scored = eligible.map((structure) => ({
    structure,
    score: scoreStructure(structure, { content, contentType, objective, format })
  }));
...
export function selectLayoutPlan({
  ...
  const structurePick = selectStructure({
    content, contentType, objective: brand.objective || content.objective || '',
