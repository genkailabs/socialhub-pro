...
export function buildLayoutSurface({
  ...
  for (const slot of structure?.slots || []) {
    ...
    if (component.layerType === 'media') {
      ...
      // Slot com moldura usa `contain` (foto inteira, sobra borda) e não `cover`.
      // Testado: `cover` escala a mídia para COBRIR o retângulo, então ela fica
      // maior que o slot — e como `surface.bg` é uma mídia só, posicionada e sem
      // recorte, o excedente vaza por cima do texto vizinho. Borda é feia;
      // vazamento quebra a peça. Encher a moldura depende de recorte por slot,
      // que a superfície ainda não tem.
