...
// §14: consistência entre slides do carrossel. Slides com estruturas diferentes
// leem como quatro posts soltos, não como um carrossel.
export function validateSlideConsistency(slides = []) {
  const ids = slides.map((slide) => slide?.structureId).filter(Boolean);
  const unique = [...new Set(ids)];
  // A capa pode (e deve) ser diferente do miolo; três estruturas ou mais é que
  // quebra a leitura de sequência.
  if (unique.length > 2) {
    return { ok: false, issues: [issue('slides_inconsistentes', `O carrossel mistura ${unique.length} estruturas diferentes.`, 'Manter uma estrutura para a capa e uma para o miolo.')] };
  }
