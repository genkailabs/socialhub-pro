...
export function imageHintForSlide(slide, context = {}) {
  ...
  return {
    ...
    query,
    queryPt: termosEmPortugues(base.scene),
    avoid: base.avoid,
    source: base.source
  };
}

// Palavras que ligam a frase mas não ajudam a achar foto nenhuma.
const LIGACOES = new Set([
  'a', 'o', 'as', 'os', 'um', 'uma', 'uns', 'umas', 'de', 'do', 'da', 'dos', 'das',
  'em', 'no', 'na', 'nos', 'nas', 'e', 'ou', 'que', 'com', 'sem', 'por', 'para',
  'pra', 'ao', 'aos', 'à', 'às', 'se', 'sobre', 'entre', 'onde', 'quando', 'como',
  'ainda', 'mais', 'muito', 'bastante', 'algum', 'alguma', 'alguém', 'alguem',
  'mesmo', 'mesma', 'seu', 'sua', 'ele', 'ela', 'isso', 'este', 'esta', 'vai',
  'está', 'esta', 'ter', 'tem', 'sendo', 'ser', 'não', 'nao', 'só', 'so'
]);

/**
 * Termos de busca em português, tirados da própria cena.
 *
 * O acervo é indexado em inglês, e é por isso que a busca principal vai em
 * inglês. Mas quem lê a dica lê em português e às vezes prefere procurar
 * assim — sobretudo em banco brasileiro. Em vez de traduzir (o que exigiria um
 * dicionário que não temos), tiramos as palavras de ligação da cena: o que
 * sobra são os substantivos que descrevem a foto.
 *
 * @param {string} scene cena descrita em português
 * @returns {string} termos separados por espaço
 */
export function termosEmPortugues(scene) {
  const palavras = String(scene || '')
    .replace(/[,.;:—–()]/g, ' ')
    .split(/\s+/)
    .map((palavra) => palavra.trim())
    .filter(Boolean)
    .filter((palavra) => palavra.length > 2 && !LIGACOES.has(palavra.toLowerCase()));
