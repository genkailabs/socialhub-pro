...
/**
 * Dicas de foto que atravessam a ponte, uma por slide.
 *
 * O Studio desenha isso dentro do painel Mídia. Como é conteúdo escrito pela
 * IA, entra recortado no tamanho e sem campo inventado: o outro lado valida de
 * novo e descarta a lista inteira se algo não bater.
 */
export function safeStudioImageHints(hints) {
  if (!Array.isArray(hints)) return [];
  return hints.flatMap((hint) => {
    if (!record(hint)) return [];
    const order = Number(hint.order);
    const scene = typeof hint.scene === 'string' ? hint.scene.trim().slice(0, 400) : '';
    if (!Number.isInteger(order) || order < 1 || order > 30 || !scene) return [];
    const safe = { order, scene, query: typeof hint.query === 'string' ? hint.query.slice(0, 300) : '' };
    if (typeof hint.headline === 'string' && hint.headline.trim()) safe.headline = hint.headline.trim().slice(0, 300);
    if (typeof hint.queryPt === 'string' && hint.queryPt.trim()) safe.queryPt = hint.queryPt.trim().slice(0, 300);
    if (typeof hint.avoid === 'string' && hint.avoid.trim()) safe.avoid = hint.avoid.trim().slice(0, 300);
    return [safe];
  }).slice(0, 30);
}

export function studioImageHintsMessage({ channelId, hints }) {
  return {
    type: 'cs:image-hints',
    version: CAROUSEL_STUDIO_PROTOCOL_VERSION,
    channelId,
    hints: safeStudioImageHints(hints)
  };
}
...
export function studioInitMessage({ channelId, title, doc, brand, templateId, slideCount, script, initialMedia, theme, imageHints }) {
  const media = safeStudioInitialMedia(initialMedia);
  const hints = safeStudioImageHints(imageHints);
  return {
    type: 'cs:init',
    version: CAROUSEL_STUDIO_PROTOCOL_VERSION,
    channelId,
    ...
    imageHints: hints.length ? hints : undefined
  };
}
