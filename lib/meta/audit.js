...
// Atividade por dia da semana — alimenta o gráfico de 7 barras do diagnóstico.
// Cada dia carrega quantos posts saíram, quantas interações vieram e a hora do
// post que rendeu mais naquele dia. Zero é zero: dia sem post fica em zero, sem
// preenchimento estimado.
export function weeklyActivity(media = []) {
  const dias = Array.from({ length: 7 }, (_, weekday) => ({
    weekday,
    posts: 0,
    interactions: 0,
    peakHour: null,
  ...
  for (const m of media) {
    ...
    dia.posts += 1;
    dia.interactions += inter;
    if (inter > 0 && inter > dia._melhor) {
      dia._melhor = inter;
      dia.peakHour = data.getHours();
    }
  }

  return dias.map(({ _melhor, ...dia }) => dia);
}
...
export function buildAuditSummary({ profile = {}, media = [], followerHistory = [], insights = null, now = new Date() }) {
  ...
  return {
    ...
    weekActivity: weeklyActivity(media),
