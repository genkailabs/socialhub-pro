...
//     funcao nem o aceita.
export async function publishInstagramStory({ igId, token, imageUrl }) {
  if (!imageUrl) throw new Error('Story exige uma imagem.');
  const params = new URLSearchParams({ image_url: imageUrl, media_type: 'STORIES', access_token: token });
  const res = await (await fetch(`${GRAPH}/${igId}/media?${params}`, { method: 'POST' })).json();
