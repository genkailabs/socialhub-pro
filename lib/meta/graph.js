...
/**
 * Aguarda até que o container da mídia esteja pronto para ser publicado.
 * Aceita custom `tries` e `delay` para processamentos demorados (ex: vídeos longos).
 */
export async function waitContainerReady({ containerId, token, tries = 6, delay = 1500 }) {
...
// Publica uma imagem como Story do Instagram (MVP V2: Story e arte estatica
// 1080x1920, nao video).
//
// Duas diferencas em relacao ao feed, ambas da propria Graph API:
//   - media_type=STORIES no container;
//   - Story nao tem legenda. Mandar `caption` aqui nao da erro, so e ignorado
//     pela Meta — e um parametro que mente sobre o que vai acontecer, entao a
//     funcao nem o aceita.
export async function publishInstagramStory({ igId, token, imageUrl, videoUrl, isVideo }) {
  const mediaUrl = videoUrl || imageUrl;
  if (!mediaUrl) throw new Error('Story exige uma imagem ou vídeo.');
  const isVideoStory = isVideo === true || !!videoUrl || !!mediaUrl.match(/\.(mp4|mov)(\?.*)?$/i);
  ...
  const res = await (await fetch(`${GRAPH}/${igId}/media?${params}`, { method: 'POST' })).json();
  if (res.error) throw new Error(`Story: ${res.error.message}`);

  if (isVideoStory) {
    await waitContainerReady({ containerId: res.id, token, tries: 20, delay: 3000 });
  } else {
    await waitContainerReady({ containerId: res.id, token });
  }
  return publishContainer({ igId, token, creationId: res.id });
}

export async function publishInstagramReel({ igId, token, videoUrl, coverUrl, caption, shareToFeed, share_to_feed }) {
  if (!videoUrl) throw new Error('Reel exige um vídeo.');
  ...
  if (caption != null) params.set('caption', caption);
  if (coverUrl != null) params.set('cover_url', coverUrl);
  const share = shareToFeed !== undefined ? shareToFeed : share_to_feed;
  if (share !== undefined && share !== null) {
    params.set('share_to_feed', share ? 'true' : 'false');
  } else {
