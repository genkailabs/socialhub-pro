...
export async function getPipeline(brandId) {
  ...
  const done = [
    // Brand Kit pronto é DNA aprovado, não "existe linha em brand_kits". A linha
    // nasce do primeiro rascunho do onboarding, então `!!kit` dava a etapa por
    // cumprida com a entrevista pela metade.
    !!kit?.dna_generated_at,
    hasStrategy,
    planItems > 0,
