...
// `postsPerWeek` é opcional e existe porque a frequência que a pessoa escolhe na
// jornada é semanal (3x, 5x), e content_plans só guarda posts_per_day inteiro —
// 3x por semana não cabe lá. Sem o parâmetro, o ritmo continua vindo do banco.
export async function generateStrategy({ brandId, postsPerWeek } = {}) {
  ...
  try {
    const { data: strategy, cost } = await runSkill({
      ...
      input: {
        ...
        auditPriorities,
        postsPerWeek: Math.max(1, Math.min(21, Number(postsPerWeek) || (plan?.posts_per_day || 1) * 7)),
