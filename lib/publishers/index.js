...
const defaultGraph = { publishInstagramImage, publishInstagramCarousel, publishInstagramStory, publishFacebookPhoto };
...
const PLATFORM = {
  instagram: async ({ token, caption, urls, format, graph }) => {
    ...
    if (format === 'stories') {
      ...
      for (const url of urls) {
        ids.push(await graph.publishInstagramStory({ igId: token.platform_user_id, token: token.access_token, imageUrl: url }));
      }
...
export async function publishPostTo({ platform, token, caption, urls, format, graph = defaultGraph, retryOptions } = {}) {
  ...
  const mediaId = await withRetry(() => PLATFORM[platform]({ token, caption: caption || '', urls, format, graph }), retryOptions || {});
  return mediaId;
}
