...
//   1. Formato manda em publicabilidade (lib/formats.js). Reel e Stories nunca
//      chegam como 'scheduled' — statusAfterApproval os manda para 'ready_to_post'
//      —, mas o publicador recusa por garantia: o sistema nao pode alegar ter
//      publicado o que so entregou como roteiro (§5.1).
...
const defaultGraph = { publishInstagramImage, publishInstagramCarousel, publishFacebookPhoto };
...
const PLATFORM = {
  instagram: async ({ token, caption, urls, graph }) => {
    if (urls.length > 1) {
  ...
  facebook: async ({ token, caption, urls, graph }) => {
...
export async function publishPostTo({ platform, token, caption, urls, format, graph = defaultGraph, retryOptions } = {}) {
  ...
  const mediaId = await withRetry(() => PLATFORM[platform]({ token, caption: caption || '', urls, graph }), retryOptions || {});
  return mediaId;
}
