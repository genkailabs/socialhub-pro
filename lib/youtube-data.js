...
export async function getYoutubeFollowerHistory(brandId, days = 14) {
  ...
  const { data } = await supabase
    ...
    .limit(days);
  // Busca os N mais recentes (desc) e devolve em ordem cronológica para o gráfico.
  return (data || []).reverse();
}
...
export async function getYoutubeVideos(brandId, limit = 12) {
  ...
  const { data } = await supabase
    ...
    .select('video_id, title, published_at, views, likes, comments, avg_view_pct, watch_time_min, snapshot_date')
    .eq('brand_id', brandId)
    .order('snapshot_date', { ascending: false })
