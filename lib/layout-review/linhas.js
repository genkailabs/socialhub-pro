 ...
 *   botão na tela.
 * @param {boolean} [entrada.cortado] o conteúdo não coube na caixa e o render
 *   aparou o que sobrou. Só quem desenhou sabe disso — vem medido de fora.
 * @returns {{id:string, severidade:string, mensagem:string, correcao?:object}[]}
 */
export function revisarLinhas({ texto, linhas, largura, papel = "corpo", rotulo = "Texto", medir, cortado = false }) {
  const problemas = [];
  ...
  // 0. Texto aparado pela própria caixa: a caixa tem altura fixa e o conteúdo é
  //    mais alto, então o render corta o que sobra. É o defeito mais grave que
  //    existe num bloco de texto — some conteúdo da peça sem avisar ninguém — e
  //    quem descobre é quem desenha, medindo o transbordo. Por isso chega como
  //    fato de fora, não como conclusão daqui.
  if (cortado) {
    problemas.push(problema(
      "texto_cortado",
      SEVERIDADE.CRITICO,
      `"${rotulo}" não cabe na caixa e está sendo cortado no render.`,
      {
        correcao: {
          tipo: "reduzir_corpo",
          descricao: "Reduzir o corpo até o texto caber, ou dar mais altura à caixa.",
        },
      },
    ));
  }
