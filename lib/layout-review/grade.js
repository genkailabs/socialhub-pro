...
export function revisarGrade({ blocos = [], areaSegura = AREA_SEGURA } = {}) {
  const problemas = [];
  // Decorativo fica de fora de TODAS as regras de grade, não só da sobreposição.
  //
  // Painel, véu e divisor existem para cobrir o slide: encostar na borda é o
  // trabalho deles. Cobrá-los pela área segura acusava "o Painel de fundo
  // encosta na borda" em cada slide de cada carrossel — a categoria Grid ia a
  // zero e a nota caía por um defeito que não existe. Pela mesma razão eles não
  // entram no eixo: um painel de sangria não "divide eixo" com ninguém, e a
  // regra do elemento solto o apontaria para sempre.
  const pecas = blocos.filter((b) => b.caixa && !b.sangra && !b.decorativo);
  if (!pecas.length) return problemas;

  // 0. Fora do slide. Não é "perto da borda" — é para fora dela, e o que
  //    passou não existe na arte exportada. Vem antes da área segura porque as
  //    duas se sobrepõem e esta é a verdadeira.
  for (const peca of pecas) {
    const c = peca.caixa;
    const saiu = c.x < -0.5 || c.y < -0.5 || c.x + c.w > 100.5 || c.y + c.h > 100.5;
    if (!saiu) continue;
    problemas.push(problema(
      "fora_do_slide",
      SEVERIDADE.CRITICO,
      `"${peca.rotulo || peca.papel}" passa da borda do slide; o que sai não aparece na arte.`,
      { blocoId: peca.id, correcao: { tipo: "trazer_para_dentro", margem: areaSegura } },
    ));
  }
  const foraDoSlide = new Set(problemas.map((p) => p.blocoId));

  // 1. Área segura. Vem primeiro porque é o único defeito desta lista que o
  //    Instagram pode literalmente cortar da tela.
  for (const peca of pecas) {
    // Já foi acusada de sair do slide: repetir com uma mensagem mais fraca só
    // enche a lista.
    if (foraDoSlide.has(peca.id)) continue;
    const c = peca.caixa;
  ...
  // 5. Sobreposição entre peças de conteúdo. Decorativo já saiu lá em cima.
