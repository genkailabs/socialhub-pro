    ...
    try {
      const res = await runSkill({
        ...
        input: {
          ...
          dna,
          // Quando a peça nasceu do fluxo editorial de carrossel, o tipo diz
          // qual promessa a revisão deve cobrar.
          contentType: item.contentType || production?.editorial?.contentType || '',
...
export async function approveContent({ postId, scheduledAt = null }) {
  if (!postId) return { error: 'Conteúdo não informado.' };
  ...
  const { data: post } = await supabase.from('posts').select('id, format, brand_id, scheduled_at').eq('id', postId).maybeSingle();
  if (!post) return { error: 'Conteúdo não encontrado.' };

  const status = statusAfterApproval(post.format);
  // "Aprovar e agendar" marcava o post como agendado e deixava a data nula: ele
  // sumia da grade do Calendário e nunca era publicado, porque quem publica
  // procura por data. Sem data no post nem no formulário, a aprovação para
  // aqui e diz o que falta.
  const precisaDeData = status === 'scheduled' && !post.scheduled_at;
  const quando = scheduledAt ? new Date(scheduledAt) : null;
  if (precisaDeData) {
    if (!quando || Number.isNaN(quando.getTime())) return { error: 'Escolha o dia e a hora da publicação.' };
    // "Agora" é um pedido legítimo: o publicador roda a cada 5 minutos e leva
    // tudo que tem `scheduled_at <= now()`. Exigir futuro recusava justamente
    // quem queria publicar já. A margem cobre o minuto que se perde entre
    // escolher a hora e clicar em aprovar; data de ontem continua sendo erro.
    if (quando.getTime() < Date.now() - 5 * 60 * 1000) {
      return { error: 'Essa data já passou. Escolha outra ou use "Sair agora" para o próximo ciclo.' };
    }
  }

  const patch = precisaDeData ? { status, scheduled_at: quando.toISOString() } : { status };
