  ...
  try {
    ...
    try {
      ...
      if (gen.imageProvider === 'deapi') {
        rows.push({
          brand_id: brandId, kind: 'image', provider: 'deapi', model: gen.imageModel,
    ...
    return { ok: true, spec: gen.spec, imageUrls: gen.imageUrls, cost: gen.cost, imageProvider: gen.imageProvider };
  } catch (e) {
    // registra a falha (ex: sem crédito) — núcleo honesto
    try {
      await supabase.from('generation_jobs').insert({
        brand_id: brandId, kind: 'post', provider: 'deepseek', model: 'deepseek-chat',
