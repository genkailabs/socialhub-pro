...
import { parseChannelReport, parseVideoRows } from '@/lib/youtube/analytics';
...
// Métricas de vários vídeos numa única chamada (dimensions=video), entre startDate e endDate.
// Devolve [{ videoId, views, likes, comments, avgViewPct, watchTimeMin }].
export async function getVideosStats(accessToken, videoIds, startDate, endDate) {
  ...
  const data = await (await fetch(`${ANALYTICS}?${p}`, { headers: { Authorization: `Bearer ${accessToken}` } })).json();
  if (data.error) throw new Error(data.error.message || 'Erro no Analytics dos vídeos.');
  return parseVideoRows(data);
}
