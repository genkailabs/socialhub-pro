...
import { validateInstagramMedia, composeCaption, isTempMediaPath, calculateDeleteAfter, removeTempMedia } from '@/lib/posts-media';
import { prepareComposerMedia } from '@/lib/composer-media-render';
...
function composerProduction(editorState, sourceMediaUrls = []) {
  if (!editorState) return null;
  // O Studio é um editor independente. Só esta origem explícita pode escapar do
  // formato do Composer; qualquer outro valor recebido do cliente continua no
  // contrato visual-composer para não abrir uma superfície de escrita livre.
  const studio = editorState?.source === 'carrossel-studio';
  return {
    source: studio ? 'carrossel-studio' : 'visual-composer',
    version: studio ? 1 : 2,
    editorState,
    sourceMediaUrls: [...new Set((sourceMediaUrls || []).filter(Boolean))]
  };
}
...
async function finalizePublishedTempMedia({ supabase, postId, mediaUrls, sourceMediaUrls, coverUrl }) {
  const result = await removeTempMedia(supabase, [
    ...(Array.isArray(mediaUrls) ? mediaUrls : []),
    ...(Array.isArray(sourceMediaUrls) ? sourceMediaUrls : []),
    coverUrl
  ]);
  if (!result.ok || !result.paths.length || !postId) return result;
  ...
  const { error: postError } = await supabase
    ...
    .eq('id', postId);
  if (postError) return { ok: false, paths: result.paths, error: postError.message };

  const { error: mediaError } = await supabase
    .from('posts_media')
    .update({ deleted_at: now, updated_at: now, last_deletion_error: null })
    .eq('post_id', postId);
  if (mediaError) return { ok: false, paths: result.paths, error: mediaError.message };
  return result;
}

export async function publishNow({
  brandId, draftId = null, caption, hashtags, firstComment, imageUrls, imageUrl, format = 'image',
  ...
  let media = validateInstagramMedia(pickImages({ imageUrls, imageUrl }));
  ...
  let prepared = null;
  try {
    prepared = await prepareComposerMedia({
      supabase,
      brandId,
      imageUrls: media.urls,
      editorState
    });
    media = validateInstagramMedia(prepared.urls);
    if (!media.ok) throw new Error(media.error);
    // A capa gerada a partir do vídeo final vence a que veio do cliente: ela
    // reflete o arquivo que o Instagram vai receber (PRD Reels §2).
    ...
    const publishedRow = {
      ...
      delete_after: deleteAfter,
      ...(editorState && { production: composerProduction(editorState, prepared.sourceUrls) })
    };
    const postQuery = draftId
      ? supabase.from('posts').update(publishedRow).eq('id', draftId).eq('brand_id', brandId).in('status', ['draft', 'scheduled'])
      : supabase.from('posts').insert(publishedRow);
    const { data: postRow, error: insertError } = await postQuery.select('id').maybeSingle();
    ...
    const cleanup = await finalizePublishedTempMedia({
      supabase,
      postId: postRow?.id || null,
      mediaUrls: media.urls,
      sourceMediaUrls: prepared.sourceUrls,
      coverUrl
    });
    ...
    if (!cleanup.ok) return { ok: true, id: igMediaId, warning: `Publicado no Instagram, mas a mídia temporária ficou pendente de limpeza: ${cleanup.error}` };
    return { ok: true, id: igMediaId };
  } catch (e) {
    if (prepared?.paths?.length) {
      await removeTempMedia(supabase, prepared.paths);
    }
    return { error: e.message };
  }
}

export async function deleteComposerDraft({ brandId, draftId }) {
  if (!brandId || !draftId) return { error: 'Rascunho inválido.' };
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  ...
  const { data: draft, error: draftError } = await supabase
    .from('posts')
    .select('id, status, media_url, media_urls, cover_url, cover_storage_path, production')
    .eq('id', draftId)
    .eq('brand_id', brandId)
    .eq('status', 'draft')
    .maybeSingle();
  if (draftError) return { error: `Não foi possível localizar o rascunho: ${draftError.message}` };
  if (!draft) return { error: 'Rascunho não encontrado.' };

  const cleanup = await removeTempMedia(supabase, [
    draft.media_url,
    ...(Array.isArray(draft.media_urls) ? draft.media_urls : []),
    ...(Array.isArray(draft.production?.sourceMediaUrls) ? draft.production.sourceMediaUrls : []),
    draft.cover_url,
    draft.cover_storage_path
  ]);
  if (!cleanup.ok) return { error: `Não foi possível remover a mídia temporária: ${cleanup.error}` };

  const { error: mediaError } = await supabase.from('posts_media').delete().eq('post_id', draftId);
  if (mediaError) return { error: `Não foi possível remover os metadados da mídia: ${mediaError.message}` };
  const { error: postError } = await supabase
    .from('posts')
    .delete()
    .eq('id', draftId)
    .eq('brand_id', brandId)
    .eq('status', 'draft');
  if (postError) return { error: `Não foi possível excluir o rascunho: ${postError.message}` };
...
export async function schedulePost({
  brandId, draftId = null, caption, hashtags, firstComment = '', altText = '', imageUrls, imageUrl, scheduledAt, format = 'image',
  ...
  try {
    ...
    return { error: error.message || 'Não foi possível preparar o enquadramento da mídia.' };
  }
  ...
  const query = draftId
    ? supabase.from('posts').update(scheduledRow).eq('id', draftId).eq('brand_id', brandId).in('status', ['draft', 'scheduled'])
    : supabase.from('posts').insert(scheduledRow);
  const { data: postRow, error } = await query.select('id').maybeSingle();
  if (error) {
    if (prepared?.paths?.length) {
      await removeTempMedia(supabase, prepared.paths);
    }
  ...
  revalidatePath('/calendar');
  return { ok: true, id: postRow?.id || draftId || null };
}
...
export async function saveDraft({
  ...
  story_overlay_text, story_overlay_position, story_overlay_style,
  internal_reference_url, cover_storage_path, thumb_offset_ms, share_to_feed = true,
  editorState = null, location = '', taggedPeople = '', revalidate = true
}) {
  if (!brandId) return { error: 'Marca não selecionada.' };
  const cleanFormat = normalizeFormat(format);
  ...
  const draftRow = {
    ...
    share_to_feed: share_to_feed !== undefined ? Boolean(share_to_feed) : true,
    delete_after: deleteAfter,
    ...(editorState && { production: composerProduction(editorState, urls) })
  };
  const query = draftId
  ...
  // `revalidate: false` existe para o autosave do Carrossel Studio.
  //
  // O Studio salva a cada 700ms de digitação. Cada `revalidatePath` faz o Next
  // refazer a árvore da rota aberta, e o React DESMONTA o <iframe> do editor:
  // a arte sumia e voltava no meio da edição — era esse o "bug que apaga e
  // volta". Medido com Playwright: com revalidate, `framedetached` +
  // `frameattached` a cada save; sem ele, nenhum.
  //
  // Não perdemos nada: /calendar lê cookies, então é rota dinâmica e busca do
  // banco a cada visita. O revalidate só limpava o cache de navegação do
  // cliente, que expira sozinho — e quem publica, agenda ou aprova continua
  // revalidando, porque essas ações mudam listas que a pessoa vê logo depois.
  if (revalidate) revalidatePath('/calendar');
