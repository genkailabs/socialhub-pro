...
import { validateInstagramMedia, composeCaption, isTempMediaPath } from '@/lib/posts-media';
...
export async function publishNow({ brandId, caption, hashtags, firstComment, imageUrls, imageUrl, format = 'image', recommendationId, recommendation, approvalStatus, coverUrl = null }) {
  ...
  try {
    ...
    if (isTempMediaPath(media.urls[0])) {
      try {
        const pathMatch = media.urls[0].match(/temp\/[^?]+/);
        if (pathMatch) {
          await supabase.storage.from('media').remove([pathMatch[0]]);
        }
      } catch (err) {}
    }
    
    if (coverUrl && isTempMediaPath(coverUrl)) {
      try {
        const coverMatch = coverUrl.match(/temp\/[^?]+/);
        if (coverMatch) {
          await supabase.storage.from('media').remove([coverMatch[0]]);
        }
      } catch (err) {}
    }
...
export async function schedulePost({ brandId, caption, hashtags, imageUrls, imageUrl, scheduledAt, format = 'image', recommendationId, recommendation, approvalNotes = '', coverUrl = null }) {
  if (!brandId) return { error: 'Marca não selecionada.' };
  const cleanFormat = normalizeFormat(format);
  const media = validateInstagramMedia(pickImages({ imageUrls, imageUrl }));
  if (!media.ok) return { error: media.error };
  ...
  const { data: postRow, error } = await supabase.from('posts').insert({
    ...
    networks: ['instagram'],
    status: 'scheduled',
    scheduled_at: when.toISOString(), format: cleanFormat, recommendation_id: recommendationId || null, approval_notes: approvalNotes || null, cover_url: coverUrl || null
  }).select('id').maybeSingle();
...
export async function saveDraft({ brandId, caption, hashtags, imageUrls, imageUrl, format = 'image', recommendationId, recommendation, approvalNotes = '', coverUrl = null }) {
  if (!brandId) return { error: 'Marca não selecionada.' };
  const cleanFormat = normalizeFormat(format);
  ...
  const { data, error } = await supabase.from('posts').insert({
    ...
    networks: ['instagram'],
    status: 'draft',
    scheduled_at: null, format: cleanFormat, recommendation_id: recommendationId || null, approval_notes: approvalNotes || null, cover_url: coverUrl || null
  }).select('id').single();
...
export async function submitForApproval({ brandId, caption, hashtags, imageUrls, imageUrl, format = 'image', recommendationId, recommendation, approvalNotes = '', coverUrl = null }) {
  if (!brandId) return { error: 'Marca não selecionada.' };
  const cleanFormat = normalizeFormat(format);
  const urls = pickImages({ imageUrls, imageUrl });
