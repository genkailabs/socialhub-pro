...
import {
  ...
  clearDailyContentCleanupJobs,
  getDailyContentPackage,
  getDailyContentPackageById,
  getDailyContentCleanupJobs,
  getOwnedBrand,
...
async function serviceForRequest() {
  ...
  return createDailyContentService({
    ...
    cleanupMedia,
    getCleanupJobs: ({ brandId }) => getDailyContentCleanupJobs({ supabase, brandId }),
    clearCleanupJobs: ({ brandId, paths }) => clearDailyContentCleanupJobs({ supabase, brandId, paths }),
    clearCleanupFailure: ({ packageId, paths }) => clearDailyContentCleanup({ supabase, packageId, paths, now: now() }),
    recordCleanupFailure: ({ brandId, paths, error }) => recordDailyContentCleanupFailure({ supabase, brandId, paths, error, now: now() }),
    ...
    generateContent: async ({ brand, opportunity, context, verifiedResearch, mediaNamespace }) => {
