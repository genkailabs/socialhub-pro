...
const MANUAL_ITEM_FIELDS = ['topic', 'title', 'format', 'objective', 'pillar', 'summary', 'hook', 'cta', 'target_audience', 'estimated_duration', 'date'];
...
export async function generateWeekPlan({ brandId, weekStart }) {
  ...
  if (existingPlan) {
    ...
    const { data: ideas, error: ideasError } = await supabase
      .from('editorial_plan_items')
      .select('date, format, topic, title, objective, pillar, stage, summary, hook, cta, target_audience, estimated_duration, rationale, status, regeneration_count, production_error, position')
      .eq('plan_id', existingPlan.id)
      .eq('status', 'idea');
  ...
  const remainingSlots = Math.max(0, requestedPosts - preservedItems.length);
  if (remainingSlots === 0) {
  ...
  try {
    ...
    const { data: plano, cost } = await runSkill({
      ...
      input: {
        ...
        tone: kit?.tone || '',
        donts: kit?.donts || [],
        signals: [...signals.slice(0, preservedContext ? 4 : 5), ...(preservedContext ? [preservedContext] : [])],
    ...
    const validItems = plano.items.filter((i) => itemWithinWeek(i.date, semana));
...
export async function restorePlanItemVersion({ itemId, versionId }) {
  ...
  const { data: version, error: versionError } = await supabase
    ...
    .select('date, topic, title, format, pillar, stage, objective, summary, hook, cta, target_audience, estimated_duration, rationale')
    .eq('id', versionId)
...
export async function replacePlanItem({ itemId, instruction }) {
  ...
  const { data: item, error: itemError } = await supabase
    .from('editorial_plan_items')
    .select('id, plan_id, date, format, topic, title, objective, pillar, stage, summary, hook, cta, target_audience, estimated_duration, rationale, status, regeneration_count')
    .eq('id', itemId)
    .maybeSingle();
