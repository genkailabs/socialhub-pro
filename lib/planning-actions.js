...
import { planningSlotsFromAudit, planningSlotsLabel, resolveSuggestedTime, isDatePlannable, planTimeAfterEdit } from '@/lib/planning-times';
...
export async function generateWeekPlan({ brandId, weekStart }) {
  ...
  // Hoje só entra na janela se ainda couber uma publicação: com a janela
  // começando no dia atual (§1), planejar para um horário que já passou seria
  // entregar item nascido vencido.
  const availableDates = emptyWindowDates(semana, occupiedDates).filter((dia) => isDatePlannable(dia));
  ...
  try {
    ...
    const newItems = itens.map((i, idx) => ({
        plan_id: planRow.id,
        date: i.date,
        suggested_time: resolveSuggestedTime({ date: i.date, aiTime: i.suggestedTime, slots: posting.slots, index: idx }),
        format: i.format,
        topic: i.topic,
        title: i.title,
...
export async function createPlanItem({ planId, values }) {
  ...
  if (!isDatePlannable(row.date)) return { error: 'Esse dia já passou (ou não cabe mais nada nele). Escolha outra data.' };

  // Mesma regra da edição: o horário escolhido vale, salvo quando não cabe — e aí
  // o app conta o ajuste em vez de trocar calado.
  const horario = planTimeAfterEdit({ date: row.date, time: row.suggested_time });
  const notice = planTimeNotice(horario, row.suggested_time);
  row.suggested_time = horario.time;
  ...
  revalidatePath('/planning');
  return notice ? { ok: true, itemId: data?.id, notice, suggestedTime: row.suggested_time } : { ok: true, itemId: data?.id };
}
...
export async function replacePlanItem({ itemId, instruction }) {
  ...
  try {
    ...
    const { error: updateError } = await supabase.from('editorial_plan_items').update({
      date: item.date,
      // Trocar a ideia não pode reintroduzir horário vencido no item.
      suggested_time: resolveSuggestedTime({ date: item.date, aiTime: next.suggestedTime }) || item.suggested_time,
      format: next.format,
      topic: next.topic,
      title: next.title,
...
export async function updatePlanItem({ itemId, patch }) {
  ...
  // Data e horário andam juntos: mudar um recalcula o outro a partir de HOJE.
  // Só lemos o item quando a edição toca esses campos — o resto continua sendo
  // uma escrita direta.
  let notice = null;
  if (row.date !== undefined || row.suggested_time !== undefined) {
    const { data: atual, error: readError } = await supabase
      .from('editorial_plan_items')
      .select('date, suggested_time, position')
      .eq('id', itemId)
      .maybeSingle();
    if (readError) return { error: readError.message };
    if (!atual) return { error: 'Tema não encontrado.' };

    const date = row.date ?? atual.date;
    if (!isDatePlannable(date)) return { error: 'Esse dia já passou (ou não cabe mais nada nele). Escolha outra data.' };

    const dateChanged = row.date !== undefined && row.date !== atual.date;
    const timeChanged = row.suggested_time !== undefined && row.suggested_time !== atual.suggested_time;
    const escolhido = row.suggested_time ?? atual.suggested_time;
    const resultado = planTimeAfterEdit({
      date,
      time: escolhido,
      recalculate: dateChanged && !timeChanged,
      index: atual.position || 0
    });

    row.date = date;
    row.suggested_time = resultado.time;
    notice = planTimeNotice(resultado, escolhido);
  }
  ...
  revalidatePath('/planning');
  return notice ? { ok: true, notice, suggestedTime: row.suggested_time } : { ok: true };
}

// O app pode corrigir o horário, mas nunca em silêncio: a mensagem diz o que
// mudou e por quê. `null` quando a escolha do usuário foi mantida.
function planTimeNotice(resultado, escolhido) {
  if (!resultado.adjusted) return null;
  if (resultado.reason === 'date-changed') return `Data alterada: o melhor horário para o novo dia é ${resultado.time}.`;
  if (resultado.reason === 'too-soon') return `${escolhido} está a menos de 1 hora de agora. Ajustamos para ${resultado.time}.`;
  if (resultado.reason === 'invalid') return escolhido ? `Horário inválido. Usamos ${resultado.time}.` : `Sem horário definido: usamos o melhor do dia, ${resultado.time}.`;
  return 'Esse dia não comporta mais publicações — o item ficou sem horário.';
}
