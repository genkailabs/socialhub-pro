    ...
    try {
      ...
      for (let k = 0; k < count; k++) {
        ...
        const gen = await generateCreative({
          ...
          brief: { topic, format: plan.format || 'quote', goal: 'engajar a audiência' }
        });
        ...
        const jobs = [{
          brand_id: brand.id, kind: 'autopilot', provider: 'deepseek', model: gen.model,
          input_tokens: gen.usage.prompt_tokens || 0, output_tokens: gen.usage.completion_tokens || 0,
          cost_usd: gen.textCost, status: 'success', ref_post_id: post?.id
        }];
        if (gen.imageProvider === 'deapi') {
          jobs.push({
            brand_id: brand.id, kind: 'image', provider: 'deapi', model: gen.imageModel,
            cost_usd: gen.imageCost, status: 'success', ref_post_id: post?.id
          });
        }
        await admin.from('generation_jobs').insert(jobs);
