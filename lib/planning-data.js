...
export async function getWeekPlan(brandId, weekStart) {
  ...
  const { data: items } = await supabase
    ...
    .select('id, date, format, topic, title, objective, pillar, stage, summary, hook, cta, target_audience, estimated_duration, rationale, status, regeneration_count, production_error, position, post_id, editorial_plan_item_versions(id, version_number)')
    .eq('plan_id', plan.id)
