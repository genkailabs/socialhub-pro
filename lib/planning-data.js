...
import { normalizeSuggestedTime, resolveSuggestedTime } from '@/lib/planning-times';
...
export async function getWeekPlan(brandId, weekStart) {
  ...
  return {
    ...
    items: withVersions.map((item, idx) => {
      ...
      // O horário gravado manda: recalcular na leitura fazia a edição do usuário
      // "não pegar" — ele salvava 09:38 e o quadro seguia mostrando outro
      // horário. Quem grava (gerar plano, criar e editar item) já garante um
      // horário viável; aqui só cobrimos item antigo sem horário válido.
      const stored = normalizeSuggestedTime(item.suggested_time);
      const suggestedTime = stored || resolveSuggestedTime({ date: item.date, index: idx });
      return { ...item, suggested_time: suggestedTime, post_status: post?.status || null, post_scheduled_at: post?.scheduled_at || null };
    })
  };
}
