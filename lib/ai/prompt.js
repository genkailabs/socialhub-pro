...
function composerContextLines(composerContext) {
  ...
  const slots = Array.isArray(composerContext.recommendedSlots)
    ? composerContext.recommendedSlots
      ...
      .slice(0, 4)
    : [];
  const planItems = Array.isArray(composerContext.opportunities)
    ? composerContext.opportunities
      .filter((item) => /^(plan-|strategy-)/.test(String(item?.id || '')))
      .map((item) => safeOperationalText(item?.label || item?.topic, 120))
      .filter(Boolean)
      .slice(0, 3)
    : [];

  if (!memory && !slots.length && !planItems.length && composerContext.hasMetricSignal !== true) return [];
  return [
    '',
    ...
    ...(planItems.length ? [`- Estrategia e calendario: ${planItems.join(' | ')}`] : []),
    ...(slots.length ? [`- Horarios recomendados: ${slots.join(', ')}`] : []),
