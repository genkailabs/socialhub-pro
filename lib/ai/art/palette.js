...
export function resolveArtPalette({ kit = null, brandColor = '', niche = '' } = {}) {
  ...
  return {
    ...
    // Mesma regra para o painel. Sem isso, uma estrutura que pinta a peça
    // inteira de `surface` (a manchete, a conclusão, a citação) desenhava o
    // texto com a tinta calculada para `bg` — e a correção automática só
    // conseguia empurrar a cor até um cinza lavado, que foi o que apareceu no
    // PNG: título ilegível numa peça que passava na validação.
    onSurface: (contrastRatio('#FFFFFF', toHex(superficie)) || 0) >= (contrastRatio('#111111', toHex(superficie)) || 0) ? '#FFFFFF' : '#111111',
