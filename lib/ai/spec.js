...
export function normalizeSpec(raw) {
  ...
  return {
    ...
    imageTextPosition,
    contentDetails: o.content_details && typeof o.content_details === 'object' ? o.content_details : {},
    slides
  };
}
...
export function parseSpec(jsonText) {
  ...
  try {
    ...
    const start = typeof candidate === 'string' ? candidate.indexOf('{') : -1;
    const end = typeof candidate === 'string' ? candidate.lastIndexOf('}') : -1;
    try {
      raw = start >= 0 && end > start ? JSON.parse(candidate.slice(start, end + 1)) : null;
    } catch {
