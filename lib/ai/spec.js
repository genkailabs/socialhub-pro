...
import { resolveHighlight } from '@/lib/layouts/content';
...
export function normalizeSpec(raw) {
  ...
  const headline = clampStr(o.headline, 120) || 'Sem título';
  // O título que vai para a arte é o `imageTitle` quando existe — é sobre ele
  // que o destaque precisa incidir, senão a palavra pintada não está na peça.
  const artTitle = clampStr(o.image_title ?? o.imageTitle, 120) || clampStr(o.headline, 120);

  return {
    template,
    headline,
    subtext: clampStr(o.subtext, 220),
    // §1.2: o destaque é conteúdo, não decoração — sem ele o motor sabe pintar
    // uma palavra e nunca recebe qual. `resolveHighlight` ainda deriva do título
    // quando a IA devolve algo que não é recorte dele.
    highlight: resolveHighlight(clampStr(o.highlight, 48), artTitle),
    bullets,
