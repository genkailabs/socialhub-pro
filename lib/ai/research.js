...
// Quem publica bloqueia requisição anônima: identificar-se é o que separa 200
// de 403 quando a chamada sai de um IP de datacenter, como o de produção.
const EVIDENCE_USER_AGENT = 'Mozilla/5.0 (compatible; SocialHubBot/1.0; +https://socialhub-mvp-production.up.railway.app)';
// 5s bastava daqui; o servidor fica em San Francisco e a fonte costuma ser
// brasileira, então a viagem é mais longa e o teto curto derrubava fonte boa.
...
function requestPinnedHtml({ url, address }) {
  ...
  return new Promise((resolve, reject) => {
    const request = transport(url, {
      ...
      // Sem `user-agent`, quem está atrás de Cloudflare devolve 403 — e o
      // servidor de produção sai de um IP de datacenter, onde o bloqueio é bem
      // mais agressivo do que numa conexão doméstica. Aqui a requisição se
      // identifica pelo que é, com um link de contato.
      headers: {
...
async function enrichSource(source) {
  ...
  try {
    const evidence = await fetchEvidence(source?.url || source?.uri).catch((e) => {
      console.log('[evidencia] falhou', String(source?.uri || source?.url).slice(0, 60), '->', e.message);
      throw e;
    });
    if (!evidence) {
      console.log('[evidencia] descartada na validacao de URL', String(source?.uri || source?.url).slice(0, 60));
      return null;
    }
    ...
    if (!verified.ok) console.log('[evidencia] metadado incompleto', url.hostname, '->', verified.reason);
