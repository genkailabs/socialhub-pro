...
// chama runText e não sabe qual API está do outro lado.
//
// OpenRouter é o único provedor (decisão de 2026-08-04): DeepSeek saiu por
// escolha do dono, Groq saiu por ter um teto de 8000 tokens/min na conta que
// quebrava sob uso real. `lib/ai/deepseek.js`/`lib/ai/groq.js` continuam no
// repo, sem nenhum import de produção — dá para reativar um dia trocando só
// este mapa. Um provedor novo entra adicionando 1 linha aqui.
const ADAPTERS = {
...
export async function runText({ system, user, provider, model, temperature, jsonMode, maxTokens }) {
  ...
  try {
    // Sem `model`: o modelo pertence ao provedor, e pedir o modelo de um
    // provedor a outro é pedir um modelo que a outra API não conhece.
