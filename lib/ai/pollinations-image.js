...
// Só imagem — o texto/prompt continua vindo do OpenRouter. Server-side.
...
// A imagem vai na URL (GET /image/{prompt}). Prompt muito longo estoura o
// limite de URL (~2048) e a API responde 400. Rede de segurança: corta aqui.
...
export async function pollinationsImage({ prompt, model = POLLINATIONS_IMAGE_MODEL, width = DIM, height = DIM }) {
  ...
  const safePrompt = String(prompt).trim().slice(0, MAX_PROMPT);
  const url = `${BASE}/${encodeURIComponent(safePrompt)}?${params}`;
