...
import { estimateCostUsd, deapiImageCostUsd, geminiImageCostUsd } from '@/lib/ai/cost';
...
export async function generateNewsImageOptions({ supabase, brandId, topic, caption, direction }) {
  ...
  const results = await Promise.allSettled([1, 2, 3, 4].map(async (variant) => {
    const image = await geminiGenerateImage({
      prompt: buildNewsImagePrompt({ topic, caption, direction, variant })
    });
  ...
  return {
    ...
    imageCost: geminiImageCostUsd(imageUrls.length),
