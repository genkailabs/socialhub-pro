...
export const inputSchema = z.object({
  ...
  // Tipo de carrossel (lib/carrossel-tipos), quando o conteúdo veio do fluxo
  // editorial. Opcional: rascunho antigo e post de outro formato não têm tipo,
  // e nesses a revisão segue como antes.
  contentType: z.string().trim().max(60).optional().default('')
});
...
function buildUser(input) {
  ...
  // Cada tipo de carrossel promete uma coisa diferente. Revisar um case com a
  // régua de um educativo aprovaria case sem número e reprovaria bastidor por
  // não ensinar nada.
  const tipo = tipoPorId(input.contentType);
  if (tipo) {
    l.push(`Tipo de carrossel: ${tipo.label} (objetivo: ${tipo.objetivo}). ${tipo.promessa}`);
    l.push(`A peça só cumpre este tipo se entregar: ${tipo.pilares.map((p) => `${p.label} — ${p.pergunta}`).join(' | ')}`);
    if (tipo.limite) l.push(`Limitação conhecida deste tipo (não é defeito da peça): ${tipo.limite}`);
    l.push('');
  }
