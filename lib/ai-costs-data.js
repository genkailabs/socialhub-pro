...
export async function getAICostsSummary({ brandId = null, skillId = null, period = 'all', page = 1 } = {}) {
  ...
  const { data: aggData } = await applyFilters(
    supabase
      ...
      .select('provider, kind, cost_usd, input_tokens, output_tokens, status, created_at')
      .order('created_at', { ascending: false })
      .limit(SUMMARY_CAP)
  );

  let totalUsd = 0, textUsd = 0, imageUsd = 0, researchUsd = 0;
  let totalInputTokens = 0, totalOutputTokens = 0;
  let textCount = 0, imageCount = 0, researchCount = 0, errorCount = 0;
  // Série de gasto diário (redesign): bucketiza o conjunto agregado por dia.
  ...
  for (const job of aggData || []) {
    ...
    // Por `kind`, não por nome de provider: o provider de texto já trocou uma
    // vez (DeepSeek → Groq → OpenRouter) e bucketizar por string travava um
    // provider novo caindo, por engano, no balde "imagem".
  ...
  return {
    ...
    summary: {
      ...
      textUsd: Math.round(textUsd * 1e6) / 1e6,
