...
export async function getAICostsSummary({ brandId = null, skillId = null, period = 'all', page = 1 } = {}) {
  ...
  // Página do histórico, já com as colunas de ação/tentativa/cobrança.
  const safePage = Math.max(1, Number(page) || 1);
  const from = (safePage - 1) * COST_PAGE_SIZE;
  const { data: pageData, error } = await applyFilters(
    supabase
      .from('generation_jobs')
      .select('id, brand_id, kind, skill_id, ref_post_id, retry_attempt, charged, research_performed, provider, model, input_tokens, output_tokens, cost_usd, status, error, created_at, brands(name, color)')
