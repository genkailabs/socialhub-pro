...
export function isTempMediaPath(pathOrUrl = '') {
  ...
  return pathOrUrl.startsWith('temp/') || pathOrUrl.includes('/storage/v1/object/public/media/temp/');
}
...
export function calculateDeleteAfter(status, publishedAt, updatedAt) {
  ...
  if (['published', 'posted_manually'].includes(status)) {
    return new Date(getBaseTime(publishedAt) + 24 * 3600 * 1000).toISOString();
  }
