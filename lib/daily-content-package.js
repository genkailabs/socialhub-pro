...
function contextualCandidates(context) {
  ...
  return Array.isArray(opportunities)
    ? opportunities.filter((item) => topicOf(item) && item?.objective && item?.format)
    : [];
}
...
function toPackage(candidate, reason, slot, avoidReasons) {
  return {
    ...
    objective: String(candidate.objective || candidate.goal || '').trim(),
...
export function selectDailyOpportunity(context = {}) {
  ...
  const safe = (candidate) => {
    ...
    if (!topic || !candidate?.objective || !candidate?.format) return false;
  ...
  const opportunity = contextualCandidates(context)
    ...
    .map((candidate, index) => ({
      candidate,
      index,
      objectiveCount: objectives.get(normalize(candidate.objective || candidate.goal)) || 0,
