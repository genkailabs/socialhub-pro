...
// O horário do item já passou em São Paulo? A leitura mostra o que está gravado
// (para a edição manual valer), então a tela precisa saber avisar quando o
// horário ficou para trás — em vez de trocar o valor por baixo do usuário.
export function planTimeHasPassed(date, time, now = new Date()) {
  const dia = String(date || '').slice(0, 10);
  const hora = normalizeSuggestedTime(time);
  if (!dia || !hora) return false;
  const hoje = todayInSaoPaulo(now);
  if (dia < hoje) return true;
  if (dia > hoje) return false;
  return toMinutes(hora) <= minutesNowInSaoPaulo(now);
}

/**
 * Horário de um item depois de uma edição manual do usuário.
 *
 * Diferente de `resolveSuggestedTime` (que trata palpite de IA), aqui existe uma
 * escolha humana em jogo: ela só é trocada quando não cabe de verdade, e a troca
 * precisa ser contada para quem editou. Por isso devolve o motivo junto do
 * horário, em vez de reescrever em silêncio.
 *
 * `recalculate` liga quando a data do item mudou e o horário não: nesse caso o
 * horário antigo era o melhor para a data antiga, e o melhor da data nova é
 * calculado a partir de hoje.
 *
 * Motivos: `date-changed`, `too-soon`, `invalid`, `day-full`.
 */
export function planTimeAfterEdit({ date, time, recalculate = false, slots = FALLBACK_PLANNING_SLOTS, index = 0, now = new Date() } = {}) {
  const piso = earliestTimeFor(date, now);
  if (piso === null) return { time: null, adjusted: true, reason: 'day-full' };

  const melhor = () => suggestedTimeForDate(date, slots, index, now);
  if (recalculate) return { time: melhor(), adjusted: true, reason: 'date-changed' };

  const escolhido = normalizeSuggestedTime(time);
  if (!escolhido) return { time: melhor(), adjusted: true, reason: 'invalid' };
  if (toMinutes(escolhido) < toMinutes(piso)) return { time: melhor(), adjusted: true, reason: 'too-soon' };
  return { time: escolhido, adjusted: false, reason: null };
}

export function scheduleIsoFromPlanning(date, time, slots = FALLBACK_PLANNING_SLOTS, index = 0, now = new Date()) {
  const suggested = normalizeSuggestedTime(time);
  if (!date || !suggested) return null;
  const resolved = resolveSuggestedTime({ date, aiTime: suggested, slots, index, now }) || suggested;
