...
// O CSS limita border-radius à metade da caixa; no SVG um rx maior estica o
// canto e transforma o retângulo em elipse. Clampar mantém canvas, prévia e
// arquivo final idênticos (§16).
export function clampRadius(value, width, height) {
  return Math.max(0, Math.min(Number(value) || 0, width / 2, height / 2));
}
