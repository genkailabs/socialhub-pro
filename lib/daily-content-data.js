...
export function createDailyContentService(dependencies) {
  ...
  async function prepare({ brandId, contentDate } = {}) {
    ...
    const persistFailure = async ({ error, now }) => {
      ...
      let details = null;
      if (paths.length) {
        const safeToDelete = claimedPackage
...
export async function recordDailyContentCleanupFailure({ supabase, packageId, paths, error: cleanupError, now }) {
  const { data, error } = await supabase
    .from('daily_content_packages')
    .update({
      cleanup_pending_paths: cleanupPaths(paths),
      cleanup_error: String(cleanupError || 'Não foi possível remover as mídias geradas.').slice(0, 500),
      updated_at: now.toISOString()
    })
    .eq('id', packageId)
    .select('id')
