...
function auditOpportunities(audit) {
  ...
  return raw
    .map((item) => ({
      ...
      status: typeof item === 'object' ? item?.status : null
    }))
    ...
    .map((item, index) => ({
      ...
      provenance: { status: item.status === 'approved' ? 'approved' : 'unapproved', source: 'audit' }
    }));
}
...
function planOpportunities(planItems = []) {
  return planItems
    .filter((item) => item?.status === 'approved' && (item?.topic || item?.title))
    .slice(0, 3)
    .map((item, index) => ({
      ...
      provenance: { status: 'approved', source: 'editorial-plan' }
    }));
}

export function buildLocalOpportunities({ planItems = [], strategy = null, recentPosts = [], audit = null, niche = '', brandDna = null } = {}) {
  const planned = planOpportunities(planItems);
  const diagnostic = auditOpportunities(audit);
  const hasValidatedBrandDna = brandDna?.status === 'approved';
  const base = nicheOpportunities(niche).map((item, index) => ({
    id: `local-${index}`,
    description: 'Sugestao pronta para adaptar a sua marca',
    provenance: { status: hasValidatedBrandDna ? 'approved' : 'unapproved', source: 'brand-dna' },
    ...item
  }));
  ...
  if (objective && strategy?.status === 'approved') {
    contextual.push({
      ...
      format: 'Post para Instagram',
      goal: objective,
      provenance: { status: 'approved', source: 'content-strategy' }
    });
  }
  if (recentTopic) {
    contextual.push({
      ...
      format: 'Carrossel',
      goal: 'engajar',
      provenance: { status: 'unapproved', source: 'recent-post' }
    });
  }
  ...
  return [
    ...
    ...base,
    {
      ...
      format: 'Post para Instagram',
      goal: 'engajar',
      provenance: { status: 'unapproved', source: 'manual-suggestion' }
    }
  ];
}
...
export async function getComposerContext({ brandId, brand = {}, audit = null } = {}) {
  ...
  const fallback = {
    opportunities: buildLocalOpportunities({ audit, niche: brand.niche || brand.category, brandDna: brand.brandDna }),
    recentPosts: [],
    planItems: [],
  ...
  try {
    ...
    const [recentPosts, strategies, plans, dnaVersions] = await Promise.all([
      ...
      safeQuery(() => supabase.from('editorial_plans')
        ...
        .limit(1)),
      safeQuery(() => supabase.from('brand_dna_versions')
        .select('id, status, content, report')
        .eq('brand_id', brandId)
        .eq('status', 'approved')
        .limit(1))
    ]);
    ...
    const planItems = plan?.id
      ? await safeQuery(() => supabase.from('editorial_plan_items')
        ...
        .limit(6))
      : [];
    const usablePlanItems = filterUsablePlanItems(planItems, currentWeek);
    const linkedItems = recentPosts.length
      ? await safeQuery(() => supabase.from('editorial_plan_items')
        .select('post_id, topic, title, format, objective')
        .in('post_id', recentPosts.map((post) => post.id))
        .limit(12))
      : [];
    const itemByPostId = new Map(linkedItems.map((item) => [item.post_id, item]));
    const enrichedRecentPosts = filterCurrentWeekPublishedPosts(recentPosts, now).map((post) => {
      const linked = itemByPostId.get(post.id);
      return linked ? {
        ...post,
        objective: linked.objective || null,
        format: post.format || linked.format || post.media_type,
        topic: post.title || linked.topic || linked.title || post.content
      } : post;
    });

    return {
      opportunities: buildLocalOpportunities({
        planItems: usablePlanItems,
        strategy: activeStrategy(strategies),
        brandDna: dnaVersions[0],
      ...
      recentPosts: enrichedRecentPosts,
      planItems: usablePlanItems,
      weeklyMemory: buildWeeklyMemory(enrichedRecentPosts, now),
      ...slots
    };
  } catch {
    return fallback;
