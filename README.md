...
- A barra de onboarding (`FlowStepper`) segue a jornada real — Brand Kit → Estratégia → Planejar semana → Aprovar & agendar → Publicar — sem prometer geração automática diária. O mascote Hub (`Mascot`) caminha até a etapa em foco e explica o que fazer ali; nas telas fora do fluxo, o mesmo mascote explica a tela pelo `MascotTip`.
